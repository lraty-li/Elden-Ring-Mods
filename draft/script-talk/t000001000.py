    ...
    if not GetEstusAllocation(mode1) < 0:
        ...
        if (DoesPlayerHaveItem(3, goods5 + 0 + mode1 * 50 + z30 * 2) == 1 or DoesPlayerHaveItem(3, goods5
            ...
            if DoesPlayerHaveItem(3, goods5 + 0 + mode1 * 50 + z30 * 2) == 1:
                """State 2,4"""
                ReplaceTool(1000 + mode1 * 50 + z30 * 2 + 0,
                            1000 + mode1 * 50 + z30 * 2 + 1, 1)
            else:
                """State 3"""
                pass
            while True:
...
def t000001000_x22():
    ...
    while True:
        ...
        if IsMultiplayerInProgress() == 1:
            ...
            """State 5"""
            def WhilePaused():
                GiveSpEffectToPlayerIf(GetEventFlag(
                    9000) == 1 and not GetWorkValue(0), 9607)
                GiveSpEffectToPlayerIf(GetEventFlag(
                    9000) == 1 and GetWorkValue(0) == 10, 9608)
                GiveSpEffectToPlayerIf(GetEventFlag(
                    9000) == 1 and GetDistanceToPlayer() < 1.05, 9609)
                GiveSpEffectToPlayerIf(GetEventFlag(
                    4698) == 1 and GetEventFlag(9001) == 1, 9606)
...
def t000001000_x23():
    ...
    if call.Done():
        ...
        """State 23"""
        SetEventFlag(4698, 0)
        SetEventFlagIf(DoesSelfHaveSpEffect(9680) ==
                       1 and not GetEventFlag(953), 4698, 1)
        ...
        if GetEventFlag(4698) == 1:
            ...
            if not GetEventFlag(400001):
                ...
                assert t000001000_x35(
                    z13=20000, val4=3.5, z14=1, z15=2, z16=60)
            else:
                """State 8,12"""
                assert t000001000_x35(
                    z13=20001, val4=3.5, z14=1, z15=2, z16=60)
    else:
        """State 2"""
        pass
    """State 14"""
...
def t000001000_x31():
    ...
    while True:
        ...
        AddTalkListDataIf(not GetEventFlag(
            9411) or GetEventFlag(9412) == 1, 1, 15000420, -1)
        # action:15000540:"Level Up"
        AddTalkListDataIf(GetEventFlag(4680) ==
                          1 or GetEventFlag(4699) == 1, 2, 15000540, -1)
        """State 29"""
        # 圣杯瓶
        ...
        AddTalkListDataIf(ComparePlayerInventoryNumber(
            3, 8590, 4, 1, 0) == 1, 8, 15000530, -1)
        ...
        # ============= custom ==========
        # todo 制作及时已经癫火，仍然能聊天的版本(只检查是否通过梅琳娜烧树) 

        # 21000050	告诉梅琳娜想去圆桌厅堂

        # 该参数为对应状态调用 GetTalkListEntryResult() 会获得：11 ，见  elif GetTalkListEntryResult() == 11:

        # 设置 z8=98 能正确显示文本，但效果等同 离开
        # AddTalkListData 能无视检查根据第一个参数直接添加选项(第一个参数指定动作,第二个参数指定显示的文本(eventTextForTalk)

        # 第一个参数疑似 某 动作/选项 顺序 的映射，99意味最后一项

        # 如果梅琳娜已经出现，添加与梅琳娜对话选项

            # 使用五个event flag 达成传递选择情况 (2^5=32)，这五个flag在能够进入该函数时，值都应该确定
            ...
            # 1054539205 : 烧树发生
            # 以上event flag 在 可对话时的值应为确定的 (确定选项时4651在最低位,35009360 在最高位):
            # 4680 =1 ; 4681 =1 ; 35009360 = 0 ; 1054539201 = 0; 1054539205 = 0;

            # 11009260 : 到达黄金树根脚,暂时分别(但之后不会设置回来) (<text id="100131000">……谢谢你。</text>)
            # 11009251 : 在王城击败噩兆王后重新会合对话

            #不能为0，否则无法进入 t000003000?
            # 4651 : 梅琳娜是否在篝火处已经出现

            # 1054539200 : 通过梅琳娜触发烧树?
            # 1054539215 : 到达火焰大锅？
            # 108 已经接受癫火？ (被梅琳娜发现时无法被正确设置)


        # if( GetEventFlag(108)==0 and (not (GetEventFlag(11009260)==1 and GetEventFlag(11009251)==0))  and GetEventFlag(1054539205)==0 and GetEventFlag(1054539201)==0 and  GetEventFlag(35009360)==0 and GetEventFlag(4681)==1 and GetEventFlag(4680)==1):
        ...
        # ============================
            ...
            # 灵药圣杯瓶？
            OpenPhysickMenu()
            assert not (CheckSpecificPersonMenuIsOpen(21, 0) ==
                        1 and not CheckSpecificPersonGenericDialogIsOpen(0))
            ...
            # AddTalkListData(6, 15000395, -1)
            OpenRepository()
            assert not (CheckSpecificPersonMenuIsOpen(3, 0) ==
                        1 and not CheckSpecificPersonGenericDialogIsOpen(0))
        elif GetTalkListEntryResult() == 7:
            """State 13"""
            # 大卢恩？
            c1_137()
            assert not (CheckSpecificPersonMenuIsOpen(24, 0) ==
                        1 and not CheckSpecificPersonGenericDialogIsOpen(0))
            ...
            assert not (CheckSpecificPersonMenuIsOpen(26, 0) ==
                        1 and not CheckSpecificPersonGenericDialogIsOpen(0))
...
def t000001000_x34():
    ...
    if not GetEventFlag(4651):
        """State 3,4"""
        OpenSoul()
        assert not (CheckSpecificPersonMenuIsOpen(10, 0) ==
                    1 and not CheckSpecificPersonGenericDialogIsOpen(0))
    else:
        """State 2,1"""
        SetEventFlag(4652, 1)
...
def t000001000_x35(z13=_, val4=_, z14=1, z15=2, z16=60):
    """State 0,4"""
    assert not DoesPlayerHaveSpEffect(
        9600) and not DoesPlayerHaveSpEffect(9603)
    """State 1"""
    # c2180 梅琳娜角色id
...
def t000001000_x2222():
    # custom func of x36 invok
    # z12: 4655 梅琳娜; 4657 ranni ?
    # check t000001000_x35

    # val4 为动画时间？
    if not GetEventFlag(4651):
        assert t000001000_x35(z13=20001, val4=1, z14=1, z15=2, z16=60)
    else:
        # 梅琳娜已出现
        # 0 tpost 后瞬间消失，设计上是无动画？
        # 20010
        # 20011
        assert t000001000_x29(20010)
        pass
    return 0
...
def t000001000_x2223():
    ...
    while True:
        ...
        if not GetTalkListEntryResult() == 99:
            ...
            # 最低位(0)
            # assert t000001000_x2225(GetTalkListEntryResult()/2)
            # if(flagIsInt ==1):
            #     SetEventFlag(4651, 0)
            #     位(1)
            #     assert t000001000_x2225((GetTalkListEntryResult()/2)/2)
            #     ...
            # else:
            #     SetEventFlag(4651, 1)
            #     assert t000001000_x2225((GetTalkListEntryResult()/2-0.5)/2)
            #     ...
            if(GetTalkListEntryResult() == 1):
                # custom id decmical(1)->binary(00001)
                assert t000001000_x2226(0,0,0,0,1)
            elif(GetTalkListEntryResult() == 2):
                assert t000001000_x2226(0,0,0,1,0)
                pass
            elif(GetTalkListEntryResult() == 3):
                assert t000001000_x2226(0,0,0,1,1)
                pass
            elif(GetTalkListEntryResult() == 4):
                assert t000001000_x2226(0,0,1,0,0)
                pass
            elif(GetTalkListEntryResult() == 5):
                assert t000001000_x2226(0,0,1,0,1)
                pass
            elif(GetTalkListEntryResult() == 6):
                assert t000001000_x2226(0,0,1,1,0)
                pass
            elif(GetTalkListEntryResult() == 7):
                assert t000001000_x2226(0,0,1,1,1)
                pass
            elif(GetTalkListEntryResult() == 8):
                assert t000001000_x2226(0,1,0,0,0)
                pass
            elif(GetTalkListEntryResult() == 9):
                assert t000001000_x2226(0,1,0,0,1)
                pass
            elif(GetTalkListEntryResult() == 10):
                assert t000001000_x2226(0,1,0,1,0)
                pass
            elif(GetTalkListEntryResult() == 11):
                assert t000001000_x2226(0,1,0,1,1)
                pass
            elif(GetTalkListEntryResult() == 12):
                assert t000001000_x2226(0,1,1,0,0)
                pass
            elif(GetTalkListEntryResult() == 13):
                assert t000001000_x2226(0,1,1,0,1)
                pass
            elif(GetTalkListEntryResult() == 14):
                assert t000001000_x2226(0,1,1,1,0)
                pass
            elif(GetTalkListEntryResult() == 15):
                assert t000001000_x2226(0,1,1,1,1)
                pass
            elif(GetTalkListEntryResult() == 16):
                assert t000001000_x2226(1,0,0,0,0)
                pass
            elif(GetTalkListEntryResult() == 17):
                assert t000001000_x2226(1,0,0,0,1)
                pass
            else:
                return 0
...
# def t000001000_x2225(p0=0):
#     # 通过减1判断是否是整数
#     if(p0-1>0):
#         assert t000001000_x2225(p0-1)
#     if(p0==0):
#         # 设置一个 flagIsInt
#         return 1
#     if(p0<0):
...
def t000001000_x2226(p4,p3,p2,p1,p0):
    # 设置二进制 flag
    # 原始值 4651 =1 ; 4680 =1 ; 4681 =1 ; 1054539200 = 0 ;  1054539201 = 0 ; 35009360 = 0 ; 1054539205 = 0;

    # 4651 不能为0，否则无法进入 t000003000?
    SetEventFlag(4680, p0)
    SetEventFlag(4681, p1)
    SetEventFlag(35009360, p2)
    SetEventFlag(1054539201, p3)
    SetEventFlag(1054539205, p4)

    # 能够进入对话，癫火flag (108) 为 0 ,纯净金针治疗后108的状态？ 
    # 
    # 11009260 : 到达黄金树根脚,暂时分别(但之后不会设置回来) (<text id="100131000">……谢谢你。</text>)
    # 11009251 : 在王城击败噩兆王后重新会合对话
...
def t000001000_x2224():
    # 添加二级菜单中的对话
    # 癫火放前面
    # 注释格式: 对话 eventflag ; custom eventTextForTalk(buttom text) ; TalkMsg id (搜索时去掉最后一位); talk text
    # 用于控制对话流程的 eventFlag 在第一次对话后会被设置为1
    # 35009359; ???(99990302) ; <text id="100186000">拜托你了，能不能悬崖勒马？</text>
    AddTalkListDataIf(GetEventFlag(35009359) == 1, 17, 70000117, -1)

    # 100185000	……我再说一次──
    AddTalkListDataIf(GetEventFlag(35009353) == 1, 16, 70000116, -1)

    # 100181000	……我会准备火种。(癫火)
    AddTalkListDataIf(GetEventFlag(35009356) == 1, 15, 70000115, -1)

    # 100180000	……如果你的心向着癫火，
    AddTalkListDataIf(GetEventFlag(35009351) == 1, 14, 70000114, -1)
    ...
    AddTalkListDataIf(GetEventFlag(1043349251) == 1, 7, 70000107, -1)

    #<text id="100041000">……我知道了。我直接复诵玛莉卡的箴言：</text>
    AddTalkListDataIf(GetEventFlag(1046389202) == 1, 6, 70000106, -1)
    
    # 100104000	……我直接复诵玛莉卡的箴言：
    AddTalkListDataIf(GetEventFlag(1054559201) == 1, 5, 70000105, -1)

    # 100103000	……我直接复诵玛莉卡的箴言：
    AddTalkListDataIf(GetEventFlag(1043509201) == 1, 4, 70000104, -1)

    #<text id="100102000">……我直接复诵玛莉卡的箴言：</text>
    AddTalkListDataIf(GetEventFlag(11009256) == 1, 3, 70000103, -1)
...
def t000001000_x40():
    ...
    while True:
        ...
        if GetEventFlag(1054532702) == 1:
            ...
            call = t000001000_x65(actionbutton1=6100,
                                  actionbutton2=6101, val1=45, z7=-120)
            if call.Done():
                break
            ...
            call = t000001000_x44(actionbutton3=6102,
                                  actionbutton4=6103, val3=45)
            if call.Done():
                break
...
def t000001000_x41(actionbutton1=_, actionbutton2=_):
    ...
    if CompareBonfireLevel(0, 0) == 1:
        """State 2,4"""
        assert t000001000_x3(actionbutton1=actionbutton1,
                             flag3=6001, flag4=6000)
    else:
        """State 3,5"""
        assert t000001000_x3(actionbutton1=actionbutton2,
                             flag3=6001, flag4=6000)
    """State 6"""
    return 0
...
def t000001000_x43(z8=11, z9=12):
    ...
    if not GetEventFlag(953):
        ...
        if not GetEventFlag(4699):
            ...
            """State 8"""
            # 但 eventTextForTalk 99990301 为空
...
def t000001000_x44(actionbutton3=6102, actionbutton4=6103, val3=45):
    ...
    if GetRelativeAngleBetweenPlayerAndSelf() < val3:
        ...
        Label('L0')
        """State 3"""
        call = t000001000_x41(actionbutton1=actionbutton3,
...
def t000001000_x52():
    ...
    AddTalkListDataIf(GetEventFlag(120) == 1 and GetEventFlag(
        11102790) == 1, 41, 20010080, -1)
    """State 6"""
    return 0
    """Unused"""
    """State 2"""
...
def t000001000_x57(z8=11):
    ...
    if not GetEventFlag(4680):
        ...
        if (GetEventFlag(10009655) == 1 and not GetEventFlag(105) and not DoesPlayerHaveSpEffect(4270)
            ...
            # 21000005 与梅琳娜交谈
            c5_149(not GetEventFlag(1046389203) and not GetEventFlag(
                1046382700), z8, 21000005, -1, 0, 1)
            # 21000005 与梅琳娜交谈
            c5_149(GetEventFlag(1046389203) == 1 and not GetEventFlag(
                1046382700), z8, 21000005, -1, 0, 0)
            ...
            # 21000007 与梅琳娜交谈
            c5_149(not GetEventFlag(1043349252) and not GetEventFlag(
                1046382700), z8, 21000007, -1, 0, 1)
            # 21000007 与梅琳娜交谈
            c5_149(GetEventFlag(1043349252) == 1 and not GetEventFlag(
                1046382700), z8, 21000007, -1, 0, 0)
            ...
            # 21000009 与梅琳娜交谈
            c5_149(not GetEventFlag(1038509252) and not GetEventFlag(
                1046382700), z8, 21000009, -1, 0, 1)
            # 21000009 与梅琳娜交谈
            c5_149(GetEventFlag(1038509252) == 1 and not GetEventFlag(
                1046382700), z8, 21000009, -1, 0, 0)
            ...
            # 21000011 与梅琳娜交谈
            c5_149(not GetEventFlag(1043539252) and not GetEventFlag(
                1046382700), z8, 21000011, -1, 0, 1)
            # 21000011 与梅琳娜交谈
            c5_149(GetEventFlag(1043539252) == 1 and not GetEventFlag(
                1046382700), z8, 21000011, -1, 0, 0)
            ...
            # 21000013 与梅琳娜交谈
            c5_149(not GetEventFlag(11009257) and not GetEventFlag(
                1046382700), z8, 21000013, -1, 0, 1)
            # 21000013 与梅琳娜交谈
            c5_149(GetEventFlag(11009257) == 1 and not GetEventFlag(
                1046382700), z8, 21000013, -1, 0, 0)
            ...
            # 21000015 与梅琳娜交谈
            c5_149(not GetEventFlag(1043509202) and not GetEventFlag(
                1046382700), z8, 21000015, -1, 0, 1)
            # 21000015 与梅琳娜交谈
            c5_149(GetEventFlag(1043509202) == 1 and not GetEventFlag(
                1046382700), z8, 21000015, -1, 0, 0)
            ...
            # 21000017 与梅琳娜交谈
            c5_149(not GetEventFlag(1054559202) and not GetEventFlag(
                1046382700), z8, 21000017, -1, 0, 1)
            # 21000017 与梅琳娜交谈
            c5_149(GetEventFlag(1054559202) == 1 and not GetEventFlag(
                1046382700), z8, 21000017, -1, 0, 0)
        ...
        # eventflag:400001:lot:100010:Rold Medallion
            ...
            """State 26"""
            # 21000029	和梅琳娜交谈
            c5_149(not GetEventFlag(35009356) and not GetEventFlag(
                35009357), z8, 21000029, -1, 0, 0)
            # 21000029	和梅琳娜交谈
            c5_149(GetEventFlag(35009356) == 1 or GetEventFlag(
                35009357) == 1, z8, 21000029, -1, 0, 0)
...
def t000001000_x65(actionbutton1=6100, actionbutton2=6101, val1=45, z7=-120):
    ...
    if RelativeAngleBetweenTwoPlayers_SpecifyAxis(z7) < val1:
        ...
        Label('L0')
        """State 3"""
        call = t000001000_x41(actionbutton1=actionbutton1,
...
def t000001000_x69(z5=1, z6=15000370):
    ...
    SetWorkValue(1, GetEstusAllocation(0) + GetEstusAllocation(1) + -4)
    """State 6"""
    assert t000001000_x21(z18=1, z19=1, z20=2, z21=2, z22=3,
                          z23=3, z24=4, z25=4, z26=5, z27=5, z28=1)
    """State 3"""
    # goods:10010:Golden Seed
